...
function [psthY, spkCnt, centered_bins] = plotPSTHSlidingAlpha(spikes,binsize,ntrials,fs,endt,newWind,stepsize,alpha)
...
window = @(x) max(0,alpha^2.*x.*exp(-alpha.*x));
...
for k = 1:length(sTimes)
    ...
    spkCnt(k) = sum(spike_interval.*window(time_diff));
